...
interface Props {
  ...
  onMerge: (existingItemId: string, newQuantity: number) => Promise<void>
}
...
export default function ItemFormDialog({ open, item, categories, onClose, onSubmit, onMerge }: Props) {
  const [form, setForm] = useState<FormState>(EMPTY_FORM)
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState('')
  const [mergeDialogOpen, setMergeDialogOpen] = useState(false)
  const [mergeTarget, setMergeTarget] = useState<Item | null>(null)
  const [pendingPayload, setPendingPayload] = useState<CreateItemPayload | null>(null)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!item) {
      ...
      setError('')
      try {
        const { items: similar, total } = await itemApi.findSimilar(payload.name, payload.spec)
        if (total > 0) {
          setMergeTarget(similar[0])
          setPendingPayload(payload)
          setMergeDialogOpen(true)
          return
        }
      } catch {
        // If check fails, proceed to create normally
      } finally {
        setSubmitting(false)
      }
    }
  ...
  async function handleMerge() {
    if (!mergeTarget || !pendingPayload) return
    setMergeDialogOpen(false)
    setSubmitting(true)
    setError('')
    try {
      await onMerge(mergeTarget.id, pendingPayload.quantity ?? 1)
      onClose()
    } catch (err) {
  ...
  return (
    <>
      <Dialog open={open && !mergeDialogOpen} onOpenChange={(v) => !v && onClose()}>
        <DialogContent className="sm:max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 py-2">
            ...
            {/* 规格 */}
            <div className="space-y-1.5">
              <Label htmlFor="item-spec">规格</Label>
              <Input
                id="item-spec"
                value={form.spec}
                onChange={(e) => set('spec', e.target.value)}
                placeholder="如：500ml、1kg（可选）"
              />
            </div>
      ...
      {mergeTarget && pendingPayload && (
        <MergeConfirmDialog
          open={mergeDialogOpen}
          existingItem={mergeTarget}
          incomingItem={pendingPayload}
