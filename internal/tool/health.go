...
func (h *HealthTool) CheckService(ctx context.Context, _ service.Actor, _ map[string]any) (string, error) {
	if h.Client == nil {
	...
	if err != nil {
		return "❌ 健康检查失败：请求构造错误", nil
	}

	resp, err := h.Client.Do(req)
	if err != nil {
		return "❌ 服务不可达，请检查后端服务是否正常运行。", nil
	}
	defer resp.Body.Close()

	if resp.StatusCode == http.StatusOK {
		return "✅ HomeStock 服务运行正常。", nil
	}
	return fmt.Sprintf("⚠️ 服务响应异常，状态码: %d", resp.StatusCode), nil
}
