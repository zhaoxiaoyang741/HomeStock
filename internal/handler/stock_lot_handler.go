...
func NewStockLotHandler(
    ...
    return &StockLotHandler{db: db, repo: repo, materialRepo: materialRepo, moveRepo: moveRepo, auditRepo: auditRepo}
}
...
func (h *StockLotHandler) Inbound(c *gin.Context) {
    ...
    err = h.db.Transaction(func(tx *gorm.DB) error {
        ...
        if err := moveRepo.Create(&model.StockMovement{TenantID: tenantID, MaterialID: material.ID, LotID: lot.ID, MovementType: "inbound", QuantityDelta: req.Quantity, Unit: unit, Reason: "inbound", Channel: actor.Channel, UserName: actor.UserName, UserID: actor.UserID, Remark: strings.TrimSpace(req.Notes)}); err != nil { return err }
        createdLot, err = lotRepo.Get(lot.ID, tenantID); if err != nil { return err }
...
func (h *StockLotHandler) Update(c *gin.Context) {
    ...
    if err != nil { handleStockLotRepositoryError(c, err, "get stock lot failed"); return }

    before := *lot
    if lot.ExpireAt != nil { t := *lot.ExpireAt; before.ExpireAt = &t }
    if lot.PurchasedAt != nil { t := *lot.PurchasedAt; before.PurchasedAt = &t }

    if req.ExpireAt != nil { expireAt, err := parseNullableRFC3339(*req.ExpireAt); if err != nil { httpresp.Error(c, http.StatusBadRequest, err.Error()); return }; lot.ExpireAt = expireAt }
    if req.PurchasedAt != nil { purchasedAt, err := parseNullableRFC3339(*req.PurchasedAt); if err != nil { httpresp.Error(c, http.StatusBadRequest, err.Error()); return }; lot.PurchasedAt = purchasedAt }
    if req.Location != nil { lot.Location = strings.TrimSpace(*req.Location) }
    if req.Notes != nil { lot.Notes = strings.TrimSpace(*req.Notes) }

    if err := h.repo.Update(lot); err != nil { handleStockLotRepositoryError(c, err, "update stock lot failed"); return }
...
func (h *StockLotHandler) Adjust(c *gin.Context) {
    ...
    err = h.db.Transaction(func(tx *gorm.DB) error {
        ...
        if req.TargetQuantity == 0 { lot.Status = "depleted" } else { lot.Status = "active" }
        if err := lotRepo.Update(lot); err != nil { return err }
        if delta != 0 {
            if err := moveRepo.Create(&model.StockMovement{TenantID: tenantID, MaterialID: lot.MaterialID, LotID: lot.ID, MovementType: "adjustment", QuantityDelta: delta, Unit: lot.Unit, Reason: strings.TrimSpace(req.Reason), Channel: actor.Channel, UserName: actor.UserName, UserID: actor.UserID, Remark: strings.TrimSpace(req.Remark)}); err != nil { return err }
        }
...
func resolveInboundMaterial(repo *repository.MaterialRepository, tenantID string, req *inboundStockLotRequest) (*model.Material, error) {
    if materialID := strings.TrimSpace(req.MaterialID); materialID != "" { return repo.Get(materialID, tenantID) }
    ...
    material = &model.Material{TenantID: tenantID, Name: name, Spec: spec, CategoryID: strings.TrimSpace(req.CategoryID), DefaultUnit: strings.TrimSpace(req.Unit)}
